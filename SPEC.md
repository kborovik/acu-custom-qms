...
## §I INTERFACES
- pkg: `Lab5_QMS_Customization.zip` → `_project/ProjectMetadata.xml` 22.200.001 + `_project/SiteMap.xml` (Inventory workspace QM101000 QM201000 QM301000 QM302000 QM401000; ! QMS MUIWorkspace; ! QM000000) + `Lab5.QMS.dll` + Pattern B `FrontendSources/screen/src/screens/QM/<ScreenID>/` HTML+TS + Pattern A `FrontendSources/screen/src/screens/IN/IN202500/extensions/IN202500_QMS.html|.ts` + GenericInquiryScreen QM401000 + `CreateQMSTables.sql`; ! Pages_QM ASPX; ! Role `UsersInRoles` `RolesInGraph`
...
- screen: Inventory workspace only — `QM.10.10.00` Quality Preferences (Preferences), `QM.20.10.00` Inspection Plans (Profiles), `QM.30.10.00` Inspection Orders (Transactions), `QM.30.20.00` Non-Conformance Reports (Transactions), `QM.40.10.00` Quality Queue GI (Inquiries); ScreenIDs QM101000 QM201000 QM301000 QM302000 QM401000; Pattern B PXScreen HTML+TS four QM forms; Pattern A IN202500_QMS Usr* fields; ! QMS MUIWorkspace; ! QM000000; ! Configuration; ! Classic Pages_QM ASPX; ScreenId URLs open Modern UI
...
T31|.|patch `_project/SiteMap.xml` Inventory workspace QM101000 QM201000 QM301000 QM302000 QM401000 (Preferences Profiles Transactions Transactions Inquiries); drop MUIWorkspace QMS + QM000000; ! Configuration membership|V16,I.screen,I.pkg
T32|.|add Pattern B HTML+TS four QM forms `FrontendSources/screen/src/screens/QM/<ScreenID>/`; class name = ScreenID; graphType `Lab5.QMS.QMSSetupMaint` `QMSInspectionPlanMaint` `QMSInspectionOrderEntry` `QMSNonConformanceEntry`; keep `<Page Type="Page" ScreenID="QM…">`; Evaluate ReleaseLot CloseNCR DispositionRTV Notes Files stay|V17,I.screen,I.pkg
T33|.|add Pattern A `IN202500_QMS` HTML+TS `FrontendSources/screen/src/screens/IN/IN202500/extensions/`; show `UsrQMSInspectionRequired` `UsrQMSInspectionPlanID` `UsrMinShelfLifeDays`; Quality tab ? `visible.bind` not `if.bind`|V17,I.stock,I.dac
T34|.|add GenericInquiryScreen `QM401000` Quality Queue + sitemap Inquiries; row = Inspection Order still work (lot QC Hold or order ! Completed or related NCR ! Closed); drills QM301000 order + QM302000 NCR; GI ! Evaluate ReleaseLot CloseNCR DispositionRTV|V16,V1,V6,I.screen,I.pkg
T35|.|drop `Pages_QM/*.aspx` and `Pages/QM/*.aspx` File items from zip|V17,I.pkg
T36|.|seed RolesInGraph Accessrights=4 `QM401000` for Administrator and Quality Manager|V10,I.role
T37|.|e2e prove after publish: Inventory lists Quality Preferences Inspection Plans Inspection Orders Non-Conformance Reports Quality Queue; Search finds five titles; workspace bar ! QMS tile; Configuration ! list those screens; ScreenId URLs QM101000 QM201000 QM301000 QM302000 open Modern; Evaluate ReleaseLot CloseNCR DispositionRTV Notes Files work; Quality Queue lists work rows + drills; Stock Items IN202500 shows three Usr* fields; Site Map ! QM000000; zip GenericInquiryScreen QM401000 + Pattern B + Pattern A + ! Pages_QM ASPX|V16,V17,V10,I.screen,I.stock,T31,T32,T33,T34,T35,T36
