...
V20: entitymapping-before-wait — `publish_package` after publishEnd: `_ensure_qms_detail_mappings` then `_ensure_qm_aspx_pages` (scp skip SHA-256 match) then recycle if maps inserted then `wait_published`; 26.101 publish ! nested Tests/Results EntityMapping; InspectionPlan GET 200 JSON array requires those rows + `Pages/QM/*.aspx`; aspx recopy after wait retriggers ASP.NET compile (closes §B.10, §B.12)
...
T50|.|wait_published emit start + poll last-GET heartbeat; recycle wait_rest 120s; aspx SHA-256 skip before wait; ssh_run SSH_TIMEOUT; reuse wait session|V19,V20,I.cmd,B12
