...
- cmd: `acuqms` Click console script on installable `acuqms` (import pkg `acuqms`) → subcommands `build` `publish` `seed` `deploy`; no subcommand → Click help exit 0 (not deploy); `build` writes `Lab5_QMS_Customization.zip` (SSH compile `Lab5.QMS.dll` via `acuqms/dll.py` when C# stale); `deploy` build zip + CustomizationApi publish + post-publish Role seed + `UsrQMSSetup` (QORD QNCR) per company when missing; `publish`/`deploy` `--timeout` default 900s bounds CustomizationApi publishEnd poll only; `wait_published` default 600s after publishEnd (after `_ensure_qms_detail_mappings` + recycle if inserted) and after app-pool recycle; wait timeout RuntimeError ! last InspectionPlan GET status or body kind; build/publish/seed/deploy emit per-step progress on stderr (step, target, result, elapsed); stdout stays path / status / `seeded`; skip already-published only when current tenant GET `/entity/QMS/22.200.001/InspectionPlan` 200 JSON array (empty OK); skip then seed then re-check live; after deploy GET `/entity` lists `QMS/22.200.001`; ! root `dll.py`; ! console `lab5-qms`; ! pkg `lab5_qms`
...
T47|.|swap pkg+console `lab5-qms`/`lab5_qms` → `acuqms`; Click `pack` → `build`; move `dll.py` → `acuqms/dll.py` (drop shebang); Makefile AGENTS README tests follow|I.cmd,V18,V21
