...
## §I INTERFACES
- pkg: `Lab5_QMS_Customization.zip` → `_project/ProjectMetadata.xml` 22.200.001 (template; ! live `{ver}`) + zip `project.xml` / CustomizationApi `projectDescription` `Lab5.QMS {ver}; QMS customization 22.200.001; assembly Lab5.QMS.dll; zip Lab5_QMS_Customization.zip [sha256:{member digest}]` + `_project/SiteMap.xml` (Inventory workspace QM101000 QM201000 QM301000 QM302000 QM401000; SelectedUI=D; ! QMS MUIWorkspace; ! QM000000) + `Lab5.QMS.dll` + Pattern B `PerTenantFile` zip members `screens/QM/<ScreenID>/` HTML+TS (repo `QMS/FrontendSources/screen/src/development/screens/QM/`) + Pattern A `PerTenantFile` `screens/IN/IN202500/extensions/IN202500_QMS.html|.ts` (repo `QMS/FrontendSources/screen/src/development/screens/IN/IN202500/extensions/`) + GenericInquiryScreen QM401000 (inspectable zip member; live GI = post-publish SQL seed) + `CreateQMSTables.sql` (repo `QMS/SQL/CreateQMSTables.sql`; zip `Scripts/CreateQMSTables.sql`) + File items `Pages/QM/*.aspx` (REST host; repo `QMS/Pages/QM/`); C# `QMS/Lab5.QMS/`; package XML `QMS/_project/`; zip ! File items for HTML+TS; zip ! `<Page>`; ! Role `UsersInRoles` `RolesInGraph`
- cmd: `acuqms` Click console script on installable `acuqms` (import pkg `acuqms`) → subcommands `build` `publish` `seed` `deploy` `unpublish`; no subcommand → Click help exit 0 (not deploy); `build` writes `Lab5_QMS_Customization.zip` (SSH compile `Lab5.QMS.dll` via `acuqms/dll.py` when C# stale); `deploy` build zip + CustomizationApi publish + post-publish Role seed + `UsrQMSSetup` (QORD QNCR) per company when missing; `unpublish` CustomizationApi unpublish Lab5.QMS only (AcuBootstrap stays); `ACU_SSH` set → drop leftover classes + restore OOTB IN202500+GenericInquiry HTML to site vendor (! `npm run build` production) + recycle; no-SSH → CustomizationApi only (document); ! Windows state-cache wipe; ! drop CNBN stock; `publish`/`deploy` `--timeout` default 900s bounds CustomizationApi publishEnd poll only; `wait_published` default 600s after publishEnd (after `_ensure_qms_detail_mappings` + `_ensure_qm_aspx_pages` SHA-256 skip + recycle); import+publish always recycle before `wait_published` (nested maps already present still recycle); `_recycle_app_pool` `wait_rest` 120s not `wait_published`; InspectionPlan 500 OptimizedExport NRE after first recycle → recycle again then `wait_published`; `wait_published` emit start + poll heartbeat last GET kind (heartbeat ! completion 4-col schema); reuse session across polls; `ssh_run` ! `SSH_TIMEOUT`; wait timeout RuntimeError ! last InspectionPlan GET status or body kind; build/publish/seed/deploy/unpublish emit per-step progress on stderr (step, target, result, elapsed); stdout stays path / status / `seeded` / `unpublished`; skip already-published only when current tenant GET `/entity/QMS/22.200.001/InspectionPlan` 200 JSON array (empty OK); skip then seed then re-check live; after deploy GET `/entity` lists `QMS/22.200.001`; ! root `dll.py`; ! console `lab5-qms`; ! pkg `lab5_qms`
...
V16: inventory-hosted-qm — → `.spec/check-extras.md §V16`
V17: modern-ui-only — → `.spec/check-extras.md §V17`
V18: publish-skip-tenant-qms — → `.spec/check-extras.md §V18`
V19: wait-published-600s — → `.spec/check-extras.md §V19`
V20: entitymapping-before-wait — → `.spec/check-extras.md §V20`
...
V23: native-detail-identity — Pattern B QM201000 Tests + QM301000 Results hide LineNbr (native PO301000 analog); TestID human-key first; DAC LineNbr `PXUIField Visible=false`; REST `QMS/22.200.001` Tests/Results LineNbr stay; empty `<qp-grid>` dumps TS view fields so view class ! LineNbr; Classic ASPX ! `PXGridColumn DataField="LineNbr"`
V24: receipt-doc-link — QM301000 QM302000 ReceiptNbr DisplayName PurchaseReceipt + AllowEdit opens PO Receipt PO302000; QMSInspectionOrder.PlanID DisplayName InspectionPlan (selector stay, ! require AllowEdit); QMSInspectionPlan.PlanID stay PlanID; Stock Item UsrQMSInspectionPlanID already InspectionPlan; REST field names ReceiptNbr PlanID stay
V25: displayname-pascal — PXUIField DisplayName PascalCase ! space slash period hyphen; closed abbr ID CD Nbr Qty Amt UOM Min Max Pct Date Time NCR QA QMS; uniquely names field on that screen (Order → InspectionOrder); default = DAC or action member name; qualify iff member ambiguous (ReceiptNbr → PurchaseReceipt; order PlanID → InspectionPlan); REST names stay
V26: unpublish-lab5-only — → `.spec/check-extras.md §V26`
V27: pkg-description-version — → `.spec/check-extras.md §V27`

## §T TASKS
## archived: §T.1..§T.11 → SPEC.archive.md (11 rows)
...
T30|x|e2e prove after publish: workspace bar or More Items shows Quality Management; Search finds Quality Preferences Inspection Plans Inspection Orders Non-Conformance Reports; Site Map Workspaces `QM.00.00.00` `QM.10.10.00` `QM.20.10.00` `QM.30.10.00` `QM.30.20.00`; ScreenId URLs keep working|V16,I.screen,T29
...
T34|x|add GenericInquiryScreen `QM401000` Quality Queue + sitemap Inquiries; row = Inspection Order still work (lot QC Hold or order ! Completed or NCR ! Closed); one row per order (`GIGroupBy` `Order.inspectionOrderNbr`); drills QM301000 + QM302000; GI ! Evaluate ReleaseLot CloseNCR DispositionRTV|V16,V1,V6,I.screen,I.pkg
...
T37|x|e2e prove after publish: Inventory lists five QM titles; Search finds them; workspace bar ! QMS tile; ScreenId URLs open Modern; Quality Queue work rows + drills; IN202500 Usr* fields; Site Map ! QM000000; zip Pattern B/A + Pages/QM ASPX + ! `<Page>`|V16,V17,V10,I.screen,I.stock,T31,T32,T33,T34,T35,T36
...
T57|x|add Click `unpublish` + `gmake unpublish`: CustomizationApi unpublish Lab5.QMS only (AcuBootstrap stays); `ACU_SSH` set → drop leftovers + restore OOTB vendor HTML (! `npm run build` production) + recycle; no-SSH CustomizationApi-only; ! Windows state-cache wipe; ! drop CNBN stock|V26,I.cmd,B16
T58|x|unit unpublish merge list keeps AcuBootstrap + import-after-unpublish ! skip; e2e after unpublish: SM204505 ! Lab5.QMS; GET `/entity` ! QMS; InspectionPlan ! 200 JSON array; IN202500 ! UsrQMS; GenericInquiry vendor = IN202000; stock stay; subsequent deploy imports + InspectionPlan 200 JSON array|V26,V18,I.cmd,T57,B16
...
T61|x|e2e published Description starts `Lab5.QMS {package_version()}` + 22.200.001 + Lab5.QMS.dll + `[sha256:`|V27,I.pkg,T59
...
B16|2026-09-12|unpublish `npm run build --env production screenIds=IN202500` rewrites GenericInquiry.html + vendors; Stock Items GI IN2025PL blank|V26
