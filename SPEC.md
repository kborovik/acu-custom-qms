...
T32|x|add Pattern B HTML+TS four QM forms `FrontendSources/screen/src/screens/QM/<ScreenID>/`; class name = ScreenID; graphType `Lab5.QMS.QMSSetupMaint` `QMSInspectionPlanMaint` `QMSInspectionOrderEntry` `QMSNonConformanceEntry`; keep `<Page Type="Page" ScreenID="QM…">`; Evaluate ReleaseLot CloseNCR DispositionRTV Notes Files stay|V17,I.screen,I.pkg
...
T34|x|add GenericInquiryScreen `QM401000` Quality Queue + sitemap Inquiries; row = Inspection Order still work (lot QC Hold or order ! Completed or related NCR ! Closed); drills QM301000 order + QM302000 NCR; GI ! Evaluate ReleaseLot CloseNCR DispositionRTV|V16,V1,V6,I.screen,I.pkg
T35|x|drop `Pages_QM/*.aspx` and `Pages/QM/*.aspx` File items from zip|V17,I.pkg
