...
## §I INTERFACES
- pkg: `Lab5_QMS_Customization.zip` → `_project/ProjectMetadata.xml` 22.200.001 + `_project/SiteMap.xml` (Inventory workspace QM101000 QM201000 QM301000 QM302000 QM401000; SelectedUI=D; ! QMS MUIWorkspace; ! QM000000) + `Lab5.QMS.dll` + Pattern B `PerTenantFile` zip members `screens/QM/<ScreenID>/` HTML+TS (repo `FrontendSources/screen/src/screens/QM/`) + Pattern A `PerTenantFile` `screens/IN/IN202500/extensions/IN202500_QMS.html|.ts` (repo `FrontendSources/screen/src/screens/IN/IN202500/extensions/`) + GenericInquiryScreen QM401000 (inspectable zip member; live GI = post-publish SQL seed) + `CreateQMSTables.sql` + File items `Pages/QM/*.aspx` (REST host; repo `Pages_QM/`); zip ! File items for HTML+TS; zip ! `Pages_QM/` members; zip ! `<Page>`; ! Role `UsersInRoles` `RolesInGraph`
- cmd: `lab5-qms` Click console script on installable `lab5-qms` → subcommands `pack` `publish` `seed` `deploy`; no subcommand → Click help exit 0 (not deploy); `deploy` pack `Lab5_QMS_Customization.zip` + CustomizationApi publish + post-publish Role seed + `UsrQMSSetup` (QORD QNCR) per company when missing; `publish`/`deploy` `--timeout` default 900s bounds CustomizationApi publishEnd poll only; `wait_published` default 600s after publishEnd (after `_ensure_qms_detail_mappings` + recycle if inserted) and after app-pool recycle; wait timeout RuntimeError ! last InspectionPlan GET status or body kind; pack/publish/seed/deploy emit per-step progress on stderr (step, target, result, elapsed); stdout stays path / status / `seeded`; skip already-published only when current tenant GET `/entity/QMS/22.200.001/InspectionPlan` 200 JSON array (empty OK); skip then seed then re-check live; after deploy GET `/entity` lists `QMS/22.200.001`
...
V17: modern-ui-only — zip ships Pattern B HTML+TS four QM forms as `PerTenantFile` `screens/QM/<ScreenID>/` (repo `FrontendSources/screen/src/screens/QM/`; class name = ScreenID; graphType `Lab5.QMS.QMSSetupMaint` `QMSInspectionPlanMaint` `QMSInspectionOrderEntry` `QMSNonConformanceEntry`) + Pattern A `IN202500_QMS` `PerTenantFile` `screens/IN/IN202500/extensions/` showing `UsrQMSInspectionRequired` `UsrQMSInspectionPlanID` `UsrMinShelfLifeDays`; publish copies to `customizationScreens/<tenant>/screens` + webpack `Scripts/Screens/<tenant>/<ScreenID>.html`; zip File items `Pages/QM/*.aspx` (REST graph host; repo source `Pages_QM/`; zip ! `Pages_QM/` members); zip ! File items for HTML+TS; zip ! `<Page>` items; SiteMap SelectedUI=D (! E Classic lock); ScreenId URLs open Modern UI; Quality tab ? `visible.bind` not `if.bind`; GitOps StockItem persist stays §V.13
...
T32|x|add Pattern B HTML+TS four QM forms `FrontendSources/screen/src/screens/QM/<ScreenID>/`; class name = ScreenID; graphType `Lab5.QMS.QMSSetupMaint` `QMSInspectionPlanMaint` `QMSInspectionOrderEntry` `QMSNonConformanceEntry`; Evaluate ReleaseLot CloseNCR DispositionRTV Notes Files stay|V17,I.screen,I.pkg
T33|x|add Pattern A `IN202500_QMS` HTML+TS `FrontendSources/screen/src/screens/IN/IN202500/extensions/`; show `UsrQMSInspectionRequired` `UsrQMSInspectionPlanID` `UsrMinShelfLifeDays`; Quality tab ? `visible.bind` not `if.bind`|V17,I.stock,I.dac
...
T43|.|seed EntityMapping Tests/Results before wait_published in publish_package; recycle if inserted; unit assert call order; GET 200 after maps|V20,I.cmd,B10
